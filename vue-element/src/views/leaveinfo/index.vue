<template>
  <div class="app-container">
    <div class="filter-container">
      <el-input v-model="listQuery.tel" placeholder="name" style="width: 200px;" class="filter-item" @keyup.enter.native="handleFilter" />
    ...
    >
      ...
      <el-table-column label="内容" width="180px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.leaveTime | parseTime('{y}-{m}-{d} {h}:{i}') }}</span>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    handleDelete(row, index) {
       ...
       */
      service('leaveinfo/deleteById?id='+row.id).then(res=>{
        if(res.code == 20000){
          this.$notify({
            title: 'Success',
            message: '删除成功',
