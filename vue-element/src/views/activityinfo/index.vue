<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="主持人" width="230px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.host }}</span>
        </template>
      </el-table-column>

      <el-table-column label="活动地址" width="200px" align="center">
        <template slot-scope="{row}">
      ...
      <el-table-column label="状态" class-name="status-col" width="120px">
        <template slot-scope="{row}">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="dialogStatus==='create'?createData():updateData()"
                    v-if="dialogStatus==='update' && flag==false">
          提交
        </el-button>
      </div>
    </el-dialog>
...
export default {
  ...
  methods: {
    ...
    //审核请求
    updateStatus(row){
      service('activity/updateStatusById?id='+row.id).then(res=>{
        if(res.code == 20000){
          this.$notify({
            title: 'Success',
            message: '审核成功',
    ...
    createData() {
      ...
      service.post('activity/addInfo',this.temp).then(res=>{
        if(res.code==20000){
    ...
    updateData() {
      ...
      // debugger
      console.log(this.temp);
      service.post('activity/updateInfo',this.temp).then(res=>{
        if(res.code==20000){
            this.dialogFormVisible = false
            this.$notify({
              title: 'Success',
    ...
    handleDelete(row, index) {
      service('activity/deleteById?id='+row.id).then(res=>{
        if(res.code == 20000){
          this.$notify({
            title: 'Success',
