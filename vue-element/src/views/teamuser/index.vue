<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="性别" width="130px">
        <template slot-scope="{row}">
          <!-- <el-tag>{{ row.name | typeFilter }}</el-tag> -->
          <span>{{ row.sex }}</span>
        </template>
      ...
      <el-table-column label="电话" width="130px">
        <template slot-scope="{row}">
          <!-- <el-tag>{{ row.name | typeFilter }}</el-tag> -->
          <span>{{ row.tel }}</span>
        </template>
      ...
      <el-table-column label="邮箱" width="230px">
        <template slot-scope="{row}">
          <!-- <el-tag>{{ row.name | typeFilter }}</el-tag> -->
          <span>{{ row.email }}</span>
        </template>
      ...
      <el-table-column label="加入时间" width="250px" align="center">
        <template slot-scope="{row}">
...
export default {
  ...
  methods: {
    ...
    createData() {
      ...
      service.post('teamuser/addInfo',this.temp).then(res=>{
        if(res.code==20000){
    ...
    updateData() {
      ...
      // debugger
      console.log(this.temp);
      service.post('teamuser/updateInfo',this.temp).then(res=>{
        if(res.code==20000){
            this.dialogFormVisible = false
            this.$notify({
              title: 'Success',
    ...
    handleDelete(row, index) {
      service('teamuser/deleteById?id='+row.id).then(res=>{
        if(res.code == 20000){
          this.$notify({
            title: 'Success',
            message: '删除成功',
