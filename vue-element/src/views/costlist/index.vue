<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="消费名称" width="150px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.name }}</span>
        </template>
      </el-table-column>
  ...
  methods: {
    ...
    createData() {
       ...
       */
       ...
       service.post('costlist/addInfo',this.temp).then(res=>{
        if(res.code==20000){
    ...
    updateData() {
      ...
      service.post('costlist/updateInfo',this.temp).then(res=>{
        if(res.code==20000){
          this.dialogFormVisible = false
          this.$notify({
              title: 'Success',
    ...
    handleDelete(row, index) {
       ...
       */
      service('costlist/deleteById?id='+row.id).then(res=>{
        if(res.code == 20000){
          this.$notify({
            title: 'Success',
            message: '删除成功',
