<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="邮箱" width="100px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.email }}</span>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="70px" style="width: 400px; margin-left:50px;">
        ...
        <el-form-item label="用户类型">
          <el-select  v-model="temp.type" placeholder="Select">
  ...
  methods: {
    ...
    createData() {
       ...
       */
       ...
       debugger
       service.post('userinfo/addInfo',this.temp).then(res=>{
        if(res.code==20000){
    ...
    updateData() {
       ...
       */
      console.log(this.temp)
      service.post('userinfo/updateInfo',this.temp).then(res=>{
        if(res.code==20000){
          this.dialogFormVisible = false
          this.$notify({
              title: 'Success',
    ...
    handleDelete(row, index) {
       ...
       */
      service('userinfo/deleteById?id='+row.id).then(res=>{
        if(res.code == 20000){
          this.$notify({
            title: 'Success',
            message: '删除成功',
    ...
    updatePassWord(){
      ...
      if(this.temp2.password==this.temp2.password2){
        ...
        // const values={
        //   id:this.temp2.id,
        //   pwd:this.temp2.password
        // }
        service.post('userinfo/updatePassword',this.temp2).then(res=>{
        if(res.code==20000){
          debugger
