<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-select v-model="listQuery.team" placeholder="选择类型" clearable class="filter-item" style="width: 130px">
    ...
    >
      ...
      <el-table-column label="社团名称" width="130px">
        <template slot-scope="{row}">
      ...
      <el-table-column label="社团团长" width="130px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.userInfo.username }}</span>
        </template>
      </el-table-column>

      <el-table-column label="建团时间" width="180px" align="center">
        <template slot-scope="{row}">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="70px" style="width: 400px; margin-left:50px;">
        ...
        <el-form-item label="社团团长" >
          <el-select v-model="temp.userId" class="filter-item" placeholder="选择团长">
  ...
  methods: {
    ...
    handleAvatarSuccess(res, file) {
      ...
      beforeAvatarUpload(file) {
        ...
        if (!isJPG) {
          this.$message.error('上传头像图片只能是png或jpeg格式!');
        }
        if (!isLt2M) {
    ...
    createData() {
      ...
      service.post('http://localhost:9999/team/addInfo',this.temp).then(res=>{
        if(res==20000){
    ...
    updateData() {
      ...
      // debugger
      console.log(this.temp);
      service.post('http://localhost:9999/team/updateInfo',this.temp).then(res=>{
        if(res.code==20000){
            this.dialogFormVisible = false
            this.$notify({
              title: 'Success',
    ...
    handleDelete(row, index) {
      service('http://localhost:9999/team/deleteByID?id='+row.id).then(res=>{
        if(res.code == 20000){
          this.$notify({
            title: 'Success',
